...
class BaseModelEntity:
  ...
  def fit(self, X: BaseDataMatrixEntity, y: BaseDataArrayModel) -> None:
    self.model.fit(X, y)
    
    
  def evaluate(self, X: BaseDataMatrixEntity, y: BaseDataArrayModel) -> List(float):
    return self.model.evaluate(X, y)
    
    
  def predict(self, X: BaseDataMatrixEntity, y: None = None) -> BaseDataArrayModel:
